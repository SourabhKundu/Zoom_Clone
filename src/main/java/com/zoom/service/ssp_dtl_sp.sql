WITH
    ...
    SELECT
      CASE
        WHEN '<P_SSP_CMMNTY_ID>' = 'User Level' THEN (
          SELECT u.user_sk
          FROM DEV_ANCHOR_ANALYTICS.WP_SRC.npf_user u
          JOIN DEV_NPF_DB.CDC_DIRECT.wpa_user_appl ua
  ...
  v_ssp_count AS (
    ...
    FROM DEV_NPF_DB.CDC_DIRECT.wpa_ssp AS ssp
    JOIN DEV_NPF_DB.CDC_DIRECT.wpa_ssp_cmnty AS cmnty
    ...
    WHERE cmnty.cmnty_cde = '<P_SSP_CMMNTY_ID>'
      ...
      AND (
        v_ssp_user.v_ssp_user_sk IS NULL
  ...
  v_ssp_content AS (
    SELECT
      ...
      CASE
        ...
        ELSE 'URL'
      END                 AS ssp_type
    FROM DEV_NPF_DB.CDC_DIRECT.wpa_ssp AS s
  ...
  v_ssp_flag AS (
    ...
    FROM DEV_NPF_DB.CDC_DIRECT.wpa_ssp_user AS u
    JOIN v_ssp_latest AS latest
...
SELECT
  CASE
    WHEN vc.cnt >= 1 AND vf.flag_count = 0 THEN vf.flag_count
    WHEN vc.cnt >= 1 AND vf.flag_count > 0 THEN vf.flag_count
    ELSE 1
  END AS ssp_flag,

  CASE
    WHEN vc.cnt >= 1 AND vf.flag_count = 0 THEN vl.ssp_id
    ELSE NULL
  END AS ssp_id,

  CASE
    WHEN vc.cnt >= 1 AND vf.flag_count = 0 THEN vcnt.ssp_type
    ELSE NULL
  END AS ssp_type,

  CASE
    WHEN vc.cnt >= 1 AND vf.flag_count = 0 THEN
      CASE
